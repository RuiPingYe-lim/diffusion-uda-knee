...
DA-BRF 作为独立模型入口接在 K3 joint 之后，只修复 K 张 U1 候选相对源原图的残差，不恢复 U3/U5。它采用停止梯度的校准诊断排序、目标参考风格进度和残差半径三项约束；约束分支只更新修复器，分类 CE 仍可经过修复器回传 TRSC。

该模块目前只有实现，没有下游有效性结论。必须与身份映射、固定残差缩放和范数裁剪直接比较；若不能稳定超过这些简单控制，则删除。完整公式、命令、日志含义和停止标准见 [DABRF.md](DABRF.md)。
