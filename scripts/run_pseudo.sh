...
CFG=$CODE/bbdm_strict/configs/bbdm_knee_allslices_pseudo.json
CLF=$AS/exp/cls_allslices/best_checkpoint.pt
BCK=$AS/exp/bbdm_runs/bbdm_allslices_pseudo/checkpoints/latest.pt
FD=$AS/fuse_pseudo
LOG=$AS/pseudo_result.log
cd $CODE/bbdm_strict
exec > >(tee -a $LOG) 2>&1
echo "############ PSEUDO-LABEL UNSUPERVISED RUN start $(date) ############"

echo "===== [1] train BBDM (label_random on PSEUDO labels) ====="
python train_strict_bbdm.py --config $CFG || { echo TRAIN_FAIL; exit 1; }

echo "===== [2] +1 translate eval (volume) [PSEUDO] ====="
echo "@@@ RESULT_1 @@@"
...
echo "===== [3] gen fusion pairs (src=mrnet_sub6k TRUE-label, tgt=kneemri_test) ====="
...
echo "===== [4] split src -> tr/va by case ====="
python - <<'PY'
import pandas as pd, numpy as np
d=pd.read_csv("/root/autodl-tmp/knee_allslices/fuse_pseudo/src/fusion_pairs.csv")
rng=np.random.RandomState(42)
if "case_id" in d.columns:
    cases=d["case_id"].unique()
    va_c=set(rng.choice(cases,size=max(1,int(len(cases)*0.15)),replace=False))
...
tr.to_csv("/root/autodl-tmp/knee_allslices/fuse_pseudo/tr.csv",index=False)
va.to_csv("/root/autodl-tmp/knee_allslices/fuse_pseudo/va.csv",index=False)
print("tr",len(tr),"va",len(va))
PY

echo "===== [5] train fusion classifier ====="
...
echo "===== [6] +1+4 fusion eval (volume) [PSEUDO] ====="
echo "@@@ RESULT_14 @@@"
